    ...
    buildTypes {
        ...
        val release by getting {
            ...
            if (rootProject.file("local.properties").exists()) {
                ...
                rootProject.file("local.properties").readLines().forEach { rawLine ->
                    val line = rawLine.trim()
                    if (line.isNotEmpty() && !line.startsWith("#") && "=" in line) {
                        val (k, v) = line.split("=", limit = 2)
                        signingProps[k.trim()] = v.trim()
                    }
                }
                val storeFile = signingProps["RELEASE_STORE_FILE"]
                val storePassword = signingProps["RELEASE_STORE_PASSWORD"]
                val keyAlias = signingProps["RELEASE_KEY_ALIAS"]
                val keyPassword = signingProps["RELEASE_KEY_PASSWORD"]
                if (storeFile != null && storePassword != null && keyAlias != null && keyPassword != null) {
                    signingConfig = signingConfigs.create("releaseSign") {
